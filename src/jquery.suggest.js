    ...
    $.fn.suggest = function (source, options) {
        ...
        return this.each(function () {
            ...
            function handleKeyUp(event) {
                ...
                if (settings.xhr.url) {
                    var params = {};
                    if (settings.xhr.paramKey) {
                        params[settings.xhr.paramKey] = needle;
                    }
                    $.getJSON(settings.xhr.url, params, createJsonResponseCallback($(this), needle));
                }
            }

            function createJsonResponseCallback($suggest, needle) {
                return function (data) {
                    if (!settings.sourceValueProperty) {
                        $suggest.trigger($.fn.suggest.EVENT_UPDATE_SUGGESTIONS, [needle, data]);
                        return;
                    }
                    var valueList = [];

                    data.map(function (result) {
                        valueList.push(result[settings.sourceValueProperty]);
                    });

                    $suggest.trigger($.fn.suggest.EVENT_UPDATE_SUGGESTIONS, [needle, valueList]);
                }
            }
